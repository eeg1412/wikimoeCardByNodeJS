...
<div class="wmcard_admincenter_common_right_body">
  <el-form :model="giveStar" label-width="140px">
    ...
    <el-form-item label="标题">
      <el-input v-model="giveStar.title" placeholder="请填写标题" clearable></el-input>
    </el-form-item>
    ...
    <el-form-item label="卡牌" v-if="giveStar.type==='card'">
        <div class="wm_admin_w250" :class="{'tc':selCardData}">
          <img :src="$wikimoecard.url+selCardData.packageId+'/'+selCardData.cardId+'.jpg'" class="wm_set_pointer mb10" width="250px" height="auto" @click="openCardSelDialog" v-if="selCardData" />
          <el-button type="primary" @click="openCardSelDialog">选择卡牌</el-button>
        </div>
    </el-form-item>
    <el-form-item label="道具" v-if="giveStar.type==='item'">
      <el-select v-model="giveStar.itemId" placeholder="请选择道具">
        <el-option
          v-for="item in itemData_"
          :key="item[0]"
          :label="item[1].name"
          :value="item[0]">
          <span><img :src="'/static/otherImg/item/'+item[0]+'.png'" height="24px" width="24px" /> {{ item[1].name }}</span>
        </el-option>
      </el-select>
    </el-form-item>
    <el-form-item label="数量">
      <el-input-number size="medium" v-model="giveStar.itemNumber" :precision="0" :step="1" :min="1" :max="99999999"></el-input-number>
    </el-form-item>
    ...
    <el-form-item>
      <el-button type="primary" @click="onSubmitPre">赠送</el-button>
    </el-form-item>
  </el-form>
  <el-dialog
    title="选择卡牌"
        ...
        <div class="wm_cardlist_select_search_body">
            <el-form :inline="true" :model="searchForm">
                ...
                <el-form-item label="出自作品">
                    <el-input v-model="searchForm.title" placeholder="请输入作品名" class="wm_cardlist_search_input" clearable>
                    </el-input>
                </el-form-item>
                <el-form-item label="角色名字">
                    <el-input v-model="searchForm.name" placeholder="请输入角色名" class="wm_cardlist_search_input" clearable>
                    </el-input>
                </el-form-item>
                <div></div>
                <el-form-item label="星星等级">
                    <el-select class="wm_cardlist_select" v-model="searchForm.star" placeholder="筛选星级">
                ...
                <el-form-item label="水晶属性">
                    <el-select class="wm_cardlist_select" v-model="searchForm.cry" placeholder="筛选水晶属性">
                    <el-option label="全部" value="0"></el-option>
                ...
                <el-form-item label="被动属性">
                    <el-select class="wm_cardlist_select" v-model="searchForm.leftType" placeholder="筛选被动属性">
                    <el-option label="全部" value="0"></el-option>
                ...
                <el-form-item label="主动技能">
                    <el-select class="wm_cardlist_select" v-model="searchForm.rightType" placeholder="筛选主动技能">
                    <el-option label="全部" value="0"></el-option>
                ...
                <el-form-item>
                    <el-button type="primary" @click="searchChanged">搜索</el-button>
                </el-form-item>
            </el-form>
        </div>
        <div class="wm_mycard_list" v-if="cardBook.length>0" ref="cardList">
            <div class="wm_market_mycard_item type_mobile type_admin_sel_card" v-for="(item,index) in cardBook" v-bind:key="index" @click="selCard(item.cardId,item.packageId)">
                <img class="wm_getcard_img" :src="$wikimoecard.url+item.packageId+'/'+item.cardId+'.jpg'" :key='item.cardId'>
            </div>
        </div>
        <div class="common_nocard_tips" v-show="cardBook.length<=0&&!loading">该条件下暂无卡牌！</div>
        <div class="wm_market_content_page" v-if="cardBook.length>0">
            <el-pagination
            small
...
<style scoped>
.wm_handbook_cardlist .wm_mycard_list .wm_market_mycard_item.type_admin_sel_card{
  opacity: 1;
}
.wm_mycard_list{
