            ...
            <el-form-item label="立绘图片">
              <el-upload action=""
                ...
                <el-tooltip class="item"
                            :enterable="false"
                            effect="dark"
                            content="推荐尺寸：宽度395px-790px，高度556px-1112px"
                            placement="top">
    ...
    <div class="mb20 wm_crearcard_info_body"
      ...
      <el-row :gutter="20">
        <el-col :sm="6"
                ...
                class="mb10">
          <el-card>
            <div class="tc mb5">{{$moment(item.date).format('ll LTS')}}</div>
            <div class="ellipsis w_10">出自：{{item.title}}</div>
            <div class="ellipsis w_10">名字：{{item.name}}</div>
            <div>状态：{{item.check===false?'未审核':'已审核'}}</div>
            <div v-if="item.check===false">结果：--</div>
            <div v-else>结果：<span v-if="item.pass===false"
...
export default {
  ...
  methods: {
    getCardPackage () {
      authApi.searchcardpackage().then(res => {
        ...
        if (res.data.code == 0) {
          ...
          this.seledCardPackage = this.cardPackage[0] ? this.cardPackage[0]._id : "未设置卡包";
        }
      });
    },
    logPageChange () {
    ...
    creatCardNoText () {
      console.log(this.sprite.titleSprite)
