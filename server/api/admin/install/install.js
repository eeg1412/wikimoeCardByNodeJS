    ...
    if (global.myAppConfig.init) {
        ...
        if (req.body.config) {
            ...
            try {
                ...
                if (!validator.isInt(config_.deminingCount.toString(), { min: 1, max: 20 })) {
                    res.send({
                        code: 0,
                        msg: '最多同时存在的矿场必须大于1小于20'
                    });
                    console.info(
                        chalk.yellow(IP + '最多同时存在的矿场必须大于1小于20')
                    );
                    return false;
                }
            ...
            let opt = {
                ...
                deminingCount: Number(config_.deminingCount),//同时开启的矿场数量
            ...
            for (var index in cardData_) {
                ...
                const cardData = await cardDataDatabase.saveCardData(cardDataParams).catch((err) => {
                    res.send({
                        code: 0,
                        msg: '内部错误请联系管理员！'
                    });
