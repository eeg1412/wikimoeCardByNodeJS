...
var userStatistics = require('../../utils/database/userStatistics');
...
module.exports = async function (req, res, next) {
    ...
    if (type === 'get') {
        ...
        let UCCData = await userCreatCardData.findUserCreatCard(20, page, {}, { 'check': 1, 'date': -1 }).catch((err) => {
            res.send({
                code: 0,
                msg: '内部错误请联系管理员！'
            });
        ...
        let pass = req.body.pass ? true : false;
        let id = req.body.id;
        ...
        if (pass) {
            ...
            let packageData = await cardPackageData.findCardPackageOne({ _id: packageId }).catch((err) => {
                res.send({
                    code: 0,
                    msg: '内部错误请联系管理员！'
                });
            ...
            const saveCard = await cardData.saveCardData(cardParams).catch((err) => {
                res.send({
                    code: 0,
                    msg: '内部错误请联系管理员！'
                });
            ...
            newCardId = saveCard._id;
            // 文件转移
            fs.renameSync('./public/userCreatCard/' + UCCData._id + '.jpg', './public/card/cardIMG/' + newCardId + '.jpg')

            //增加卡包数量
            let starAdd = {}
            switch (UCCData.star) {
                case 1:
                    starAdd = { $inc: { oneStar: 1, allCardCount: 1 } };
                    break;
                case 2:
                    starAdd = { $inc: { twoStar: 1, allCardCount: 1 } };
                    break;
                case 3:
                    starAdd = { $inc: { threeStar: 1, allCardCount: 1 } };
                    break;
                case 4:
                    starAdd = { $inc: { fourStar: 1, allCardCount: 1 } };
                    break;
                case 5:
                    starAdd = { $inc: { fiveStar: 1, allCardCount: 1 } };
                    break;
                case 6:
                    starAdd = { $inc: { sixStar: 1, allCardCount: 1 } };
                    break;
            }
            await cardPackageData.updataCardPackage({ _id: packageId }, starAdd).catch((err) => {
                res.send({
                    code: 0,
                    msg: '内部错误请联系管理员！'
                });
            ...
            console.log(userParams, updateParams)
            ...
            await userStatistics.updateOne({ _id: UCCData.userID.statistics }, updateParamsUCC).catch((err) => {
                res.send({
                    code: 0,
                    msg: '内部错误，更新失败！'
                });
        ...
        //写入用户卡牌申请
        await userCreatCardData.updataUserCreatCard({ _id: id },
            {
                check: true,
                pass: pass,
                packageId: pass ? packageId : null,
                cardID: pass ? newCardId : null,
                mark: mark
            }
        ).catch((err) => {
            res.send({
        ...
        let logObj = {
            text: '使用管理员账号' + adminAccount + '使用了不正确的审卡参数！',
            ip: IP
        }
        adminUtils.adminWriteLog(logObj);
        console.info(
