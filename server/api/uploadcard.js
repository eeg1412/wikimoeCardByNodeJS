...
module.exports = async function (req, res, next) {
    ...
    let rtReg = /^[1-7]{1}$/
    let cryReg = /^[1-5]{1}$/
    if (!validator.isMongoId(packageId + '') || !starReg.test(star) || !ltReg.test(leftType) || !rtReg.test(rightType) || !cryReg.test(cry) || !title || !name || !imgBase64) {
        console.info(
            chalk.yellow(IP + '参数有误！')
        )
        res.send({
    ...
    const populate = [{ path: 'statistics' }];
    let result = await utils.tokenCheckAndEmail(token, populate).catch((err) => {
        throw err;
    });
    if (!result) {
        res.send({
    ...
    //卡牌收集大于50
    if (result.statistics.cardIndexCount <= 25) {
        res.send({
            code: 0,
