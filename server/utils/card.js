...
exports.checkPackage = async function (packageId, packageOpen) {
    let resaultPackage = await cardPackageData.findCardPackageOne({ _id: packageId }).catch((err) => {
        console.error(
            chalk.red(err)
        );
        return false;
...
// 抽卡
exports.chioseCard = async function (packageId, times = 0, baodi = false, packageOpen = 'open') {
    // 检查卡包
    ...
    // 拉取卡包下的卡牌信息
    let cardDataArr = await cardData.findCardDataMany({ packageId: packageId, disabled: false }).catch((err) => {
        console.error(
            chalk.red(err)
        );
        return false;
